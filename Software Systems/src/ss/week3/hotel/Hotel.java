...
public class Hotel {
	...
	public Bill getBill(String guestName, int nights, PrintStream stream) {
		Bill bill = new Bill(stream);
		Room room = getRoom(guestName);
		
		if (room instanceof PricedRoom) {
			PricedRoom proom = (PricedRoom) room;
			for (int i = 0; i < nights; i++) {
				bill.addNewItem(proom);
			}
			
			if (room.getSafe().isActive() && room.getSafe() instanceof PricedSafe) {
				PricedSafe psafe = (PricedSafe) room.getSafe();
				bill.addNewItem(psafe);
			}
